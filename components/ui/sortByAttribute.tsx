  ...
  function FilterButton({selectedFilter, setSelectedFilter} : {selectedFilter : String, setSelectedFilter: React.Dispatch<React.SetStateAction<string>>}){
      ...
      <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className="text-white rounded-full border border-gray-700 bg-[#2a2a2a] flex justify-start"
            >
          ...
          {/*Popover that appears when user clicks on button.*/}
          <PopoverContent className="w-60 bg-[#2a2a2a] text-white border border-gray-700 p-2" side="bottom" align="start">
            ...
            {options.map((option) => (
                <button
                    ...
                    >
                    {option} {selected === option ? <Check className="w-4 h-4 mt-auto mb-auto ml-auto text-green-500" /> : ""}
                </button>
          ))}
            </div>
          </PopoverContent>
